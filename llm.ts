...
import { GameObject, Turn } from "./model/interfaces";
...
export const llm = async (
  ...
  let prompt = `This is a choose your own adventure story called ${
    gameObject.title
  } with a theme of ${gameObject.theme} and a setting of ${
    gameObject.setting
  } from the perspective of a singular character.  There are ${
    gameObject.maxTurns
  } turns in this story.  We are currently on turn ${
    gameObject.currTurn
  }.  Use this as context for the next stage of the story have been: ${JSON.stringify(
    history
...
function initializeLLM(gameObject: GameObject) {
  gameObject.title = "Lost in the Woods";
  (gameObject.gameHistory = [
    {
      content:
        "You wake up in a dense forest, the scent of pine heavy in the air. A storm rumbles in the distance, and the only shelter in sight is an old, abandoned cabin. As you step forward, you notice four paths leading in different directions.",
      choice: "Seek shelter and explore what’s inside",
    },
  ]),
